<template>
	<el-form ref="formRef" label-position="top" require-asterisk-position="right" :model="form" @submit.prevent="onSubmit">
		<el-row :gutter="10">
			<el-col v-for="term of TERMS " :key="term.label" :span="24 / TERMS.length">
				<el-card :header="term.label" :class="[isTermCurrent(term.label) ? 'terms__current' : '', 'terms__card']"
					@click="currentTermLabel = term.label">
					<h3 style="margin-bottom: 6px;">Ставка: {{ term.base_deposit_rate_label }}</h3>
					<div v-for="option of term.desc.split('; ')" :key="option">
		...
		<section style="margin-top: 16px;">
			<el-row justify="end">
				<el-button type="success" native-type="submit" :disabled="!currentTermLabel.length">
...
const onSubmit = async () => {
	...
	emit('submit', { base_deposit_rate_label: TERMS.find(term => isTermCurrent(term.label)).base_deposit_rate })
}
