<template>
	<el-dialog :model-value="modelValue" :title="payment.name" modal-class="payment-modal" width="30%" @closed="closeModal">
		<el-form ref="formRef" require-asterisk-position="right" label-position="top" :model="form" :rules="formRules"
			@submit.prevent="onSubmit">
			<el-form-item label="Организация" prop="payment_org">
				<el-select v-model="form.payment_org" style="width: 100%;">
					<el-option v-for="item of payment.options" :key="item" :label="item" :value="item" />
				</el-select>
			</el-form-item>

			<el-form-item
				:label="`Счет списания${form.balance_account_name ? ` - ${store.account[form.balance_account_name]} руб.` : ''}`"
				prop="balance_account_name">
				<el-select v-model="form.balance_account_name" style="width: 100%;">
					<el-option v-for="item of balanceAccounts" :key="item.name" :label="item.label" :value="item.name" />
				</el-select>
			</el-form-item>

			<el-form-item label="Сумма" prop="cost">
				<el-input v-model.number="form.cost" :formatter="onlyNumbersFormatter" />
			</el-form-item>
		</el-form>
