<template>
  ...
  >
    ...
    >
      ...
      <el-form-item
        :label="`Счет списания${
          form.balance_account_name
            ? ` - ${store.account[form.balance_account_name]} руб.`
            : ''
        }`"
...
onMounted(() => {
  balanceAccounts.value.push({ name: 'primary_balance', label: 'Баланс - Основной счет' })
...
const costFieldValidator = (rule, value, callback) => {
  if (!form.value.balance_account_name) callback(new Error('Не выбран счет'))

  const balance = store.account[form.value.balance_account_name]

  if (form.value.cost === 0) callback(new Error('Некорректная сумма'))

  if (form.value.cost > balance) callback(new Error('Недостаточно средств'))
